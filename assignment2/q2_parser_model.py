...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        word_embeddings = tf.get_variable('embeddings', initializer=self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        W1 = tf.get_variable('hidden_weights', (self.config.n_features * self.config.embed_size,
                self.config.hidden_size), initializer=xavier_initializer)
        b1 = tf.get_variable('hidden_bias', (self.config.hidden_size), initializer=xavier_initializer)
        W2 = tf.get_variable('out_weights', (self.config.hidden_size, self.config.n_classes),
                initializer=tf.zeros_initializer)
        b2 = tf.get_variable('out_bias', (self.config.n_classes), initializer=tf.zeros_initializer)

        hidden = tf.nn.dropout(tf.nn.relu(tf.matmul(x, W1) + b1), self.dropout_placeholder)
        pred = tf.matmul(hidden, W2) + b2
        ### END YOUR CODE
        return pred
