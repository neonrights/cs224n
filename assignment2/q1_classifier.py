...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.float32,
                (self.config.batch_size, self.config.n_features), 'input')
        self.labels_placeholder = tf.placeholder(tf.int32,
                (self.config.batch_size, self.config.n_classes), 'output')
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        weights = tf.get_variable("weights", (self.config.n_features, self.config.n_classes), initializer=tf.zeros_initializer)
        bias = tf.get_variable("bias", (self.config.n_classes), initializer=tf.zeros_initializer)
        pred = softmax(tf.matmul(self.input_placeholder, weights) + bias)
        ### END YOUR CODE
        return pred
