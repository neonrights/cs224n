...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if word_count > max_length:
            ...
            diff = max_length - word_count
            ret.append((sentence + [zero_vector] * diff, labels + [zero_label] * diff, [True] * word_count + [False] * diff))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        glove = tf.get_variable("embeddings", initializer=self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(glove, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        state = tf.zeros(shape=(1, self.config.hidden_size))
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                preds.append(tf.matmul(tf.nn.dropout(output, self.dropout_placeholder), U) + b2)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=masked_labels, logits=masked_pred)
