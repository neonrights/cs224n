...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable('W_z', shape=(self.input_size, self._state_size), initializer=xavier_init)
            U_z = tf.get_variable('U_z', shape=(self._state_size, self._state_size), initializer=xavier_init)
            b_z = tf.get_variable('b_z', shape=(self._state_size), initializer=tf.zeros_initializer)

            W_r = tf.get_variable('W_r', shape=(self.input_size, self._state_size), initializer=xavier_init)
            U_r = tf.get_variable('U_r', shape=(self._state_size, self.state_size), initializer=xavier_init)
            b_r = tf.get_variable('b_r', shape=(self._state_size), initializer=tf.zeros_initializer)

            W_o = tf.get_variable('W_o', shape=(self.input_size, self._state_size), initializer=xavier_init)
            U_o = tf.get_variable('U_o', shape=(self._state_size, self.state_size), initializer=xavier_init)
