...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in xrange(word_count):
            window = []
            if i < window_size:
                window += [start] * (window_size - i)
            window += sentence[max(0,(i-window_size)):min(word_count, (i+window_size+1))]
            if i >= word_count - window_size:
                window += [end] * (window_size - word_count + i + 1)
            windowed_data.append(([feature for word in window for feature in word], labels[i]))
            #pdb.set_trace()
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~3-5 lines)
        word_embeddings = tf.get_variable("word_embeddings", initializer=self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("hidden_weights", shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("hidden_bias", shape=(self.config.hidden_size), initializer=tf.zeros_initializer())

        U = tf.get_variable("prediction_weights", shape=(self.config.hidden_size, self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable("prediction_bias", shape=(self.config.n_classes), initializer=tf.zeros_initializer())

        hidden = tf.nn.dropout(tf.matmul(x, W) + b1, keep_prob=self.dropout_placeholder)
        pred = tf.matmul(hidden, U) + b2
        ### END YOUR CODE
        return pred
